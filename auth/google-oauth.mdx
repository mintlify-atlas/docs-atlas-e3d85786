...
<Steps>
  ...
  <Step title="Google redirects to the callback">
    ...
    **Endpoint:** `GET /api/auth/google/callback`
    ...
    <Info>
      New Google users are created with `authProvider: 'google'` and without a `password`, `apellido`, `telefono`, or `edad`. The `apellido` field is set from `profile.name.familyName` when available, but `perfilCompleto` is always `false` for brand-new Google registrations until the user completes their profile.
    </Info>
  </Step>

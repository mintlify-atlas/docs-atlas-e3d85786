---
title: "EcoBot AI API Endpoints — POST /api/ai"
sidebarTitle: "AI Endpoints"
description: "EcoBot API endpoints: streaming text chat, image analysis, and chat history CRUD. All endpoints require JWT authentication."
---

EcoBot is Eco-It's conversational AI assistant, specialised exclusively in recycling and environmental sustainability in the Colombian context. It is powered by [OpenRouter](https://openrouter.ai) using **Google Gemini 2.5 Flash Lite** as the primary model, with an automatic fallback to `openrouter/free` when quotas are available. All endpoints under `/api/ai` require a valid JWT and apply the `verificarToken` middleware — unauthenticated requests receive `401 Unauthorized`.
...
<ResponseField name="[]._ id" type="string">

---
title: "Contact Form API Endpoint — POST /api/contact"
sidebarTitle: "Contact Endpoint"
description: "The /api/contact endpoint accepts contact form submissions from any user (logged in or guest) and sends an email notification."
---
...
<Note>
  Email delivery is handled by the **emailService** utility, which creates a Nodemailer transporter configured for **Gmail SMTP**. The sending address and password are read from the `EMAIL_USER` and `EMAIL_PASS` environment variables at runtime. The destination admin address defaults to `ecoit4167@gmail.com` and can be overridden with the `ADMIN_EMAIL` environment variable.
</Note>

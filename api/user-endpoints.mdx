...
description: "Endpoints for reading, updating, and deleting a user profile, changing passwords, and fetching public recycling map points."
---
...
<ParamField body="nuevaPassword" type="string" required>
  The desired new password. Minimum 6 characters (validation middleware enforces this; the User model requires at least 8 in production).
</ParamField>
...
curl -X PUT https://api.eco-it.app/api/user/cambiar-password \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{ "passwordActual": "MiClave2024!", "nuevaPassword": "MiClaveNueva2025!" }'
```
...
|-----------|--------|---------|
| Either field missing | `400` | `"Debes proporcionar la contraseña actual y la nueva"` |
| New password too short | `400` | `"La nueva contraseña debe tener al menos 8 caracteres"` |
